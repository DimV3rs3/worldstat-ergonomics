...
class WSErgo_City_Defaults {
	...
	/**
	 * Встроенная карта «источник → индикатор»: по одному признаку на измерение (где возможно — B&R post1990).
	 * Используется при расчёте, если в настройках карта полей пуста ({@see WSErgo_City_Bridge::get_effective_field_map()}).
	 * Веса показателей внутри оси — в {@see default_indicator_definitions()} (по умолчанию 1); веса осей в сводном E — {@see WSErgo_Model::get_weights()}.
	 *
