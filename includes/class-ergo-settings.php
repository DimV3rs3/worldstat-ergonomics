...
class WSErgo_Settings {
	...
	/**
	 * ????? ???????? ???????? ???? ????? (???? ingest) ? id ?????? CSV ? ??????? wsp_csv_datasets.
	 * ?????? ???????? ? ????? ? ????-??????????? ?? ????? ?????, ??? ??????.
	 */
	...
	/**
	 * ??????? ?????-????: ??? F, Cm, H, A, S, Ct ? ?????? ????????? { signal, invert, weight }.
	 * ?????? ??? ? ????? ? ????????????? ?????????? ????????.
	 */
	...
	public static function get_models(): array {
		...
		$default_names = [];
		foreach ( self::get_default_models() as $def ) {
			if ( ! empty( $def['id'] ) ) {
				$default_names[ (string) $def['id'] ] = (string) ( $def['name'] ?? '' );
			}
		}
		foreach ( $stored as $i => $m ) {
			if ( ! is_array( $m ) ) {
				continue;
			}
			$id = (string) ( $m['id'] ?? '' );
			if ( $id === '' || ! isset( $default_names[ $id ] ) ) {
				continue;
			}
			$name = (string) ( $m['name'] ?? '' );
			if ( $name === '' || self::is_corrupted_display_label( $name ) ) {
				$stored[ $i ]['name'] = $default_names[ $id ];
			}
		}
		return $stored;
	}
	 ...
	 * ??????? ?? ?? ? ????? ?????????? (??????? «????» ?????? ?????????).
	 */
	private static function is_corrupted_display_label( string $label ): bool {
		$label = trim( $label );
		if ( $label === '' ) {
			return false;
		}
		...
		return (bool) preg_match( '/\?{4,}/', $label ) && ! preg_match( '/[?-??-???]/u', $label );
	}
	...
	/**
	 * ?????????? ????? ??? ???????? CSV ? ??????? ????? ??????????? (??. WSErgo_Country_Macro_Calculator).
	 *
	 * @return list<string>
	 */
	 ...
	 * @return array<string, int>
	 */
	public static function normalize_city_csv_bindings_option( array $raw ): array {
		$allowed = array_flip( self::city_bindable_metric_keys() );
		$valid   = self::valid_macro_csv_source_ids();
		$out     = [];
		foreach ( $raw as $k => $v ) {
			$mk = sanitize_key( (string) $k );
	 ...
	 * @return array<string, int> metric_key => csv row id (>0)
	 */
	public static function get_city_csv_bindings(): array {
		$raw = get_option( self::OPTION_CITY_CSV_BINDINGS, null );
		if ( ! is_array( $raw ) || empty( $raw ) ) {
			$legacy = get_option( self::OPTION_CITY_MACRO_CSV_BINDINGS, [] );
			$raw    = is_array( $legacy ) ? $legacy : [];
		}
		return self::normalize_city_csv_bindings_option( $raw );
	}
